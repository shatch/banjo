...
- **`src/calendar/`** — `CalendarProvider` interface (`types.ts`), selected via `CALENDAR_PROVIDER` through `factory.ts`: `googleCalendarProvider.ts` (Google Calendar API, OAuth2 user-consent, not service-account; the default) or `caldavCalendarProvider.ts` (any CalDAV server — Fastmail, iCloud — with an app password; creates are atomic via `If-None-Match: *` on a key-derived resource name). Used only by the phone-call path, since it must act autonomously mid-call with no Claude conversation involved. This is deliberately separate from the skill's own (out-of-repo) use of Google Calendar MCP tools during online booking — the two paths can't share a client given their different execution contexts.
- **Contacts cache** (`google_contacts` table, `src/googleContacts/`) — a local mirror of the principal's address book for caller ID and `find_contact`'s fallback, filled from `CONTACTS_PROVIDER`: Google People API (`src/googleContacts/sync.ts`) or a CardDAV address book (`src/carddavContacts/`, incremental via RFC 6578 sync tokens, rows keyed `carddav:<UID>` in `googleResourceName`). `lookup.ts` and `reconcile.ts` work the same for either source. Shared WebDAV plumbing is in `src/lib/dav/`.
