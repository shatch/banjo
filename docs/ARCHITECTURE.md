...
24. **[OPEN, accepted]** A refused tool call can make the model speak during a transfer. The `call_ending` refusal (see Call transfer above) goes through the provider's `sendToolResult`, which on OpenAI Realtime always follows the result with `response.create` (`src/voice/providers/openai.ts`), so a refusal during a slow redirect can prompt the model to say something on a call that may be moments from being bridged. Nothing hangs up — the refusal only stops a second call-ending tool. Stopping the speech needs a provider-level option to deliver a tool result without starting a new response.
