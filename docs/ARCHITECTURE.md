...
24. **[FIXED, seen live]** An outbound call nobody answered left its task in `calling` until the process restarted. Banjo learned a call had ended only from the Twilio Media Stream (`stop`, or the socket closing), and an unanswered, busy, or failed call never opens one. So its `CallSession` never ended, the call stayed in the live-call registry (`src/tasks/liveCalls.ts`), and the stale-call sweep skipped it as live forever (`sweepStaleCalls`'s `getLiveCall` check). **Fix:** `originateCall` sets a Twilio `statusCallback` for the call's final status, received by `POST /telephony/twilio/status` (signature-checked like the other webhooks). `TwilioProvider.handleStatusCallback` turns `no-answer`, `busy`, `failed`, `canceled` — or `completed` without a stream ever connecting — into the normal telephony `ended` event, so the existing path unregisters the call, fails the task with a readable reason ("no one answered", "the line was busy"), and notifies the owner. A call whose stream connected is left to the stream, so a `completed` arriving before the stream's `stop` can't cut into a real conversation. **Still depends on** Twilio reaching `PUBLIC_HOSTNAME`: if the callback can't land, the old hang remains. Covered by `tests/tasks/unansweredCall.test.ts` (real provider, session, and adapter together) and `tests/telephony/twilio.test.ts`.
